...
class Librechat(App):
    ...
    class LibrechatOption(AppOption):
...
"""
            ...
            launch_option = gr.Textbox(
                label="OPENAI API TOKEN", info="请填入您的 OPENAI API TOKEN"
            )
    ...
    def render_setting_page(self):
        with gr.Blocks() as g:
            gr.Markdown("""# 设置 OPENAI API TOKEN""")
            launch_option = gr.Textbox(
                label="OPENAI API TOKEN", info="请填入您的 OPENAI API TOKEN"
            )
    ...
    def installation(self, install_location, launch_option):
        ...
        super().installation(install_location, launch_option)
        ...
        self.cfg.source_directory = os.path.join(install_location, "LibreChat")
        ...
        append_line_to_file(os.path.join(self.cfg.source_directory, '.env'), f'PORT={self.port}')
        append_line_to_file(os.path.join(self.cfg.source_directory, '.env'), f'PROXY=http://172.16.0.13:5848')
        append_line_to_file(os.path.join(self.cfg.source_directory, '.env'), f'OPENAI_API_KEY={self.cfg.launch_option}')
